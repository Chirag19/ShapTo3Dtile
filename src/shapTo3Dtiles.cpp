...
bool ShapTo3Dtiles(const char* filename, const char* dest, const char* height)
{
	...
	if (!filename || layer_id < 0 || layer_id > 10000 || !dest) {
		spdlog::info("make shpTo3dtile {0} failed", filename);
		return false;
	}

	int height_field = 50;
	if (height) {
		height_field = atoi(height);
	}
	...
	if (poDS == NULL)
	{
		spdlog::error("open shapefile {0} failed", filename);
	...
	if (!poLayer) {
		GDALClose(poDS);
		spdlog::error("open layer {0}: {1} failed", filename, layer_id);
		return false;
	}
	...
	{
		GDALClose(poDS);
		spdlog::error("only support polyon now");
		return false;
	}
	...
	if (err != OGRERR_NONE) {
		spdlog::error("no extent found in shapefile");
		return false;
	}

	if (envelop.MaxX > 180 || envelop.MinX < -180 || envelop.MaxY > 90 || envelop.MinY < -90) {
		spdlog::error("only support WGS-84 now");
		return false;
	}
	...
	/*if (!height_field.empty()) {
		...
		if (field_index == -1) {
			spdlog::warn("can`t found field {0} in {1}", height_field, filename);
		}
	}*/
	//创建tileset.json
	std::string tileset1 = "{\n\"asset\": {\n\"gltfUpAxis\": \"Z\",\n\"version\": \"0.0\"\n },\n";
	tileset1 += "\"geometricError\": 200,\n\"root\": {\n";
	double regions[6] = {0,0,0,0,0,0};
	std::string tileset2 = "\"children\": [";
	bool flag = true;
	for (auto item : items_array) {
		...
		path b3dm_file(dest);
		b3dm_file /= "tile";
		b3dm_file /= to_string(_node->_z);
		b3dm_file /= to_string(_node->_x);
		create_directories(b3dm_file);
		// fix the box 
		{
		...
		path b3dmFileData(dest);
		b3dmFileData /= "tile";
		b3dmFileData /= to_string(_node->_z);
		b3dmFileData /= to_string(_node->_x);
		b3dmFileData /= to_string(_node->_y);
		b3dmFileData += ".b3dm";
		std::string b3dm_buf = make_b3dm(v_meshes, true);
		write_file(b3dmFileData.string(), b3dm_buf.data(), b3dm_buf.size());

		char b3dm_name[512];
		path tile_json_path(dest);
		b3dmFileData /= to_string(_node->_z);
		b3dmFileData /= to_string(_node->_x);
		b3dmFileData /= to_string(_node->_y);
		b3dmFileData += ".json";
		...
		write_tileset(radian_x, radian_y,
			...
			0, max_height, 100,
			b3dm_name, tile_json_path.string(), tileset2, regions);
	}

	tileset2 += "],\n\"geometricError\": 200,\n\"refine\": \"REPLACE\"\n}\n}";

	tileset1 += " \"boundingVolume\": {\n\"region\": [\n";
	tileset1 += std::to_string(regions[0]);
	tileset1 += ",\n";
	tileset1 += std::to_string(regions[1]);
	tileset1 += ",\n";
	tileset1 += std::to_string(regions[2]);
	tileset1 += ",\n";
	tileset1 += std::to_string(regions[3]);
	tileset1 += ",\n";
	tileset1 += std::to_string(regions[4]);
	tileset1 += ",\n";
	tileset1 += std::to_string(regions[5]);
	tileset1 += "\n]\n}\n,";
	tileset1 += tileset2;
	path tilesetPath(dest);
	tilesetPath /= "tileset.json";	
	bool ret = write_file(tilesetPath.string(), tileset1.data(), tileset1.size());
	GDALClose(poDS);
	return true;
}
